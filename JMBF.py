...
### AUTHOR ILHAM X KONTOL, COLI, MR.PEJU, MEMEK, TYTYD, KONTOL X <<<<<<<<<< HEKER
########### IMPORT MODULE NGAB
import os
try:
    import requests
...
def banner():
    print("""
%s_______________  ___________ __________ ILHAM
...
%s[•]%s---------------------------------------------------%s[•]
%s |
%s[•] AUTHOR : %sMR.PEJU%s MR.AH CROT,TYTYD,MEMEK,COLI,SPERMA BASI
[•] GITHUB : https://github.com/Ilham-XD
[•] WHATSAPP : 082370472429
%s |------------------"""%(B,p,B,p,B,p,B,p,B))
...
class _start():
    ...
    def ss(self):
        ...
        if ban in ('m', 'M'):
            print("[•] Contoh password manuap %ssayang,asu123"%(B))
            while True:
